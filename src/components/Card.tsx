  ...
  return (
    <motion.div variants={childVariant} className='flex flex-col justify-between p-6 rounded-lg border dark:border-neutral-700 border-neutral-300 shadow-[0_1px_1px_rgba(0,0,0,0.05),0_4px_6px_rgba(34,42,53,0.04),0_24px_68px_rgba(47,48,55,0.05),0_2px_3px_rgba(0,0,0,0.04)] relative'>
      <div>
        ...
        <motion.p className='text-zinc-700 dark:text-zinc-300 mb-8'>{project.description}</motion.p>
      </div>
      <div className='flex gap-2 flex-wrap items-center justify-between'>
        <div className='flex gap-2 flex-wrap'>
          {<Tags items={project.tags} />}
        </div>
        {(project.liveLink || (project as any).link) && (
          <Link href={project.liveLink || (project as any).link} target={'_blank'} className='rounded-lg border border-gray-200 dark:border-neutral-800 px-3 py-2 text-sm transition-colors duration-200 hover:bg-black hover:text-white dark:hover:bg-white dark:hover:text-black text-neutral-700 dark:text-neutral-300 flex items-center gap-1'>

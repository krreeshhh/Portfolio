...
export const getSpotifyStatus = async () => {
  ...
  if (song) {
    console.log("Spotify: Song found:", song.title);
    return {
      isPlaying: true,
      song
    };
  ...
  console.log("Spotify: Nothing playing, checking recently played...");
  song = await getRecentlyPlayed(token)

  if (song) {
    console.log("Spotify: Recently played found:", song.title);
  } else {
    console.log("Spotify: No recently played found");
  }
